...
use super::{
    entity_iterator::ArchetypeResults, filter_visualizable_2d_entities, SpatialViewVisualizerData,
            ...
            |ctx, entity_path, _entity_props, spatial_ctx, results| match results {
                ArchetypeResults::LatestAt(query, results) => {
                    ...
                    let positions: &[Position2D] = {
                        let Some(positions) = results.get(Position2D::name()) else {
                            return Ok(());
                        };
                ...
                ArchetypeResults::Range(query, results) => {
                    ...
                    let positions: CachedRangeData<'_, Position2D> = {
                        let Some(positions) = results.get(Position2D::name()) else {
                            return Ok(());
                        };
